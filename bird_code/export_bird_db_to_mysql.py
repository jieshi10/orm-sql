...
conn = pymysql.connect(**conn_config, autocommit=True)
...
def convert_sqlite_type_to_postgres_type(db_type: str):
    if db_type.lower() in ['datetime', 'time', 'timestamp']:
        return db_type + '(6)'
    return db_type
...
for db_path in glob.glob(root_path):
    ...
    print(db_path, db_name)
    schema = dump_db_json_schema(f'{db_path}/{db_name}.sqlite')
    with pymysql.connect(database=db_name, **conn_config) as conn, \
        ...
        for t in schema:
            ...
            with conn.cursor() as cursor:
                ...
                if len(data) > 0:
                    cursor.executemany('insert into `' + t['name'] + '` values (' + ', '.join(['%s'] * len(data[0])) + ')', data)
